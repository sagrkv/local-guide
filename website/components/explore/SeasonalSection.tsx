            ...
            return (
              <Link key={item.id} href={`/explore/${citySlug}/collection/${item.slug}`}>
                <div
                  ...
                  style={{ backgroundColor: "var(--pm-accent, #D4A574)", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}
                >
                  {/* Subtle dot pattern */}
                  <div
                    className="absolute inset-0 opacity-[0.06] pointer-events-none"
                    style={{ backgroundImage: "radial-gradient(circle at 20% 50%, var(--pm-ink, #0D1B2A) 1px, transparent 1px)", backgroundSize: "24px 24px" }}
                  />
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {range && (
                        <span
                          ...
                          style={{ backgroundColor: "rgba(0,0,0,0.12)", color: "var(--pm-ink, #0D1B2A)" }}
                        >
                          {range}
                        </span>
                      )}
                    </div>
                    <h3 className="text-xl md:text-2xl font-bold leading-tight" style={{ fontFamily: "var(--pm-font-display)", color: "var(--pm-ink, #0D1B2A)" }}>
                      {item.title}
                    </h3>
                    {item.description && (
                      <p className="text-sm leading-relaxed max-w-lg line-clamp-2" style={{ fontFamily: "var(--pm-font-body)", color: "var(--pm-ink, #0D1B2A)", opacity: 0.75 }}>
                        {item.description}
                      </p>
                    )}
                    {spots > 0 && (
                      <span className="text-xs font-semibold" style={{ color: "var(--pm-ink, #0D1B2A)", opacity: 0.6 }}>
                        {spots} {spots === 1 ? "spot" : "spots"} to explore
                      </span>
                    )}
                  </div>
                  {/* Hover arrow */}
                  <div className="absolute right-6 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-60 transition-opacity duration-300" style={{ color: "var(--pm-ink, #0D1B2A)" }}>
