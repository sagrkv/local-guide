...
export default function CollectionEditorPage() {
  ...
  const colId = params.colId as string;
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiClient.getCollection(colId);
      const col = res.data;
      setCollection(col);
      setForm({
        title: col.title ?? "",
  ...
  const handleAddItem = async (poiId: string) => {
    try {
      setError("");
      const order = (collection?.items?.length ?? 0) + 1;
      await apiClient.addCollectionItem(colId, { poiId, order });
      setPoiSearch("");
      setPoiResults([]);
      await fetchData();
    } catch (err) {
  ...
  const handleRemoveItem = async (itemId: string) => {
    try {
      setError("");
      await apiClient.removeCollectionItem(itemId);
      await fetchData();
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      <nav className="flex items-center gap-1.5 text-[13px]">
        ...
        <ChevronRight />
        <Link href={`/${adminPrefix}/cities/${cityId}/collections`} className="text-gray-400 hover:text-white">Collections</Link>
        <ChevronRight />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr,360px] gap-4">
        ...
        <div className="border border-gray-800 rounded-lg bg-gray-900 p-4 space-y-3">
          <h2 className="text-sm font-medium text-gray-200">
            Items ({collection?.items?.length ?? 0})
          </h2>
          ...
          <div className="border-t border-gray-800/50 pt-3 space-y-2">
            <p className="text-xs font-medium text-gray-400">Add POI</p>
