...
export default function CityEditPage() {
  ...
  useEffect(() => {
    const fetchCity = async () => {
      try {
        const res = await apiClient.getCityBySlug(cityId);
        const c = res.data;
        setForm({
          name: c.name ?? "",
          tagline: c.tagline ?? "",
          ...
          state: c.state ?? "",
          country: c.country ?? "",
          centerLat: c.centerLat?.toString() ?? "",
          centerLng: c.centerLng?.toString() ?? "",
          defaultZoom: c.defaultZoom?.toString() ?? "13",
        });
      } catch (err) {
        setApiError(err instanceof Error ? err.message : "Failed to load city");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await apiClient.updateCity(cityId, parsed.data);
      router.push(`/${adminPrefix}/cities/${cityId}`);
    } catch (err) {
      setApiError(err instanceof Error ? err.message : "Failed to update city");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="max-w-2xl space-y-4">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex items-center justify-end gap-2 mt-4">
          <Link
            href={`/${adminPrefix}/cities/${cityId}`}
